...
public void run(String arg) {
	...
	gd.setInsets(0,0,0);	gd.addMessage(PLUGINNAME + ", Version " + PLUGINVERSION + ", \u00a9 2019-2025 JN Hansen", SuperHeadingFont);
	...
	gd.setInsets(0,0,0);	gd.addMessage("and an unsegmented copy of the same channel (as can be generated by CiliaQ Preparator) is required.", InstructionsFont);
	gd.setInsets(0,0,0);	gd.addMessage("Note that CiliaQ Editor will display the image in a different color scheme for you for editing:", InstructionsFont);
	gd.setInsets(0,0,0);	gd.addMessage("The segmented channel will be displayed in magenta, the unsegmented channel in green. Display range will be optimized.", InstructionsFont);
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	if(channelMask>imp.getNChannels() || channelMask < 1) {
		IJ.error("The specified segmented channel number (" + channelMask + ") is out of range.\nSelect the correct channel ID (> 0 and <= nr of channels (" + imp.getNChannels() + ")).");
		return;
	}

	if(channelForCopying>imp.getNChannels() || channelForCopying < 1) {
		IJ.error("The specified unsegmented channel number (" + channelForCopying + ") is out of range.\nSelect the correct channel ID (> 0 and <= nr of channels (" + imp.getNChannels() + ")).");
		return;
	}
